...
test.describe("회고쓰기 폼 등록 기능", () => {
  ...
  test.describe("폼 유효성 검사", () => {
    test("회고 등록 인풋이 입력되면 입력버튼이 활성화된다", async ({
      page,
    }) => {
  ...
  test.describe("회고 등록 기능", () => {
    test("회고 등록시 로컬스토리지에 데이터가 저장된다 (기존 데이터 없음)", async ({
      page,
    }) => {
      // /diaries/[id]에 접속하여 페이지 로드 확인
      await page.goto("/diaries/1");
    ...
    test("회고 등록시 기존 데이터가 있으면 기존 데이터에 추가된다", async ({
      page,
    }) => {
      // 기존 회고 데이터 설정
      await page.goto("/diaries/1");
      await page.evaluate(() => {
        ...
        localStorage.setItem(
          "retrospects",
          JSON.stringify(existingRetrospects)
        );
      });
